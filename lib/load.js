...
module.exports = function (app, options) {
    ...
    function parseRoutes(uri, json) {
        // Loops over VERBS (GET, POST, PUT... keys)
        for (key in json[uri]) {
            ...
            middleware = getMiddleware(params.policies);
            //check cors
            /*
             if (params.cors) {
            ...
            // get handler
            if (routes[route[0]]) {
                // Try the class/method way
                var controllerInstance = new routes[route[0]]();
                handler                = controllerInstance[route[1]];
            }
            else {
                // Fallback to the default, plain old express way
    ...
    function getAllMiddleWare(routeMiddleware) {
        if (globalMiddleware.length > 0) {
            ...
            // fix if policies is string and global array
    ...
    function parseMiddleware(input) {
        ...
        // prevent malformed policies routes
        ...
        if(!middleware[parts[1]]){
            var middlewareInstance = new middleware[parts[0]]();

            return middlewareInstance[parts[1]]
        }
    ...
    function setFileGlobalParams(json) {
        ...
        // check policies Path
        if (json.GLOBAL.hasOwnProperty('policiesPath')) {
            basePathPolicy = path.join(options.processdir, json.GLOBAL.policiesPath);
        }

        // check global policies and add it
        if (json.GLOBAL.hasOwnProperty('policies')) {

            if (_.isString(json.GLOBAL.policies)) {
                globalMiddleware.push(json.GLOBAL.policies);
            } else {
                globalMiddleware = json.GLOBAL.policies;
            }
        }
